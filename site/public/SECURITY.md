...
Please send a detailed mail to emanuel.rukavina.dev@pm.me to
report vulnerabilities in InterQL.

Even when unsure whether the bug in question is an exploitable
vulnerability, it is recommended to send the report to
emanuel.rukavina.dev@pm.me (and obviously not to discuss the
issue anywhere else).

Vulnerabilities are expected to be discussed _only_ on that
list, and not in public, until the official announcement on the
InterQL mailing list on the release date.
...
There are no official "Long Term Support" versions in InterQL.
...
branches. The InterQL project makes no formal guarantee for any
